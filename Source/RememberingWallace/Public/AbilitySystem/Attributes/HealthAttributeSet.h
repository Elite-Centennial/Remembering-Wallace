...
private:
	/**
	 * Gather data from the incoming data to send gameplay events to the instigator (ultimate source of the gameplay
	 * effect) and the target (owner of this instance)
	 *
	 * To receive the event data generated by this method, pass in a valid pointer to an event data struct.
	 */
	void SendGameplayEventToSourceAndTarget(
		const FGameplayEffectModCallbackData* Data,
		const FGameplayTag& TagForSource,
		const FGameplayTag& TagForTarget,
		float Magnitude,
		FGameplayEventData* OutEventData = nullptr) const;
